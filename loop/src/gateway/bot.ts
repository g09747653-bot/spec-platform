...
const ONBOARDING = [
  ...
  '',
  'Пришлите ЗАДУМКУ ПРОЕКТА — текстом или голосовым сообщением. Я покажу, что понял, и',
  'спрошу кнопкой, запускать ли: платформа развернёт задумку в спецификацию, контур соберёт',
  'проект и прогонит тесты, а сюда придут алерты о каждом шаге.',
  '',
  'Команды:',
...
export function createTelegramGateway(deps: GatewayDeps): TelegramGateway {
  ...
  const statusReport = (): string => {
    ...
    if (projects.length === 0) {
      return 'Проектов пока нет. Пришлите задумку — текстом или голосом.';
    }
  ...
  const handleMessage = async (message: NonNullable<TelegramUpdate['message']>): Promise<void> => {
    ...
    if (message.voice !== undefined) {
      if (actions.transcribe === undefined) {
        await send(
          'Голосовой путь ещё не настроен (задача 165). Пришлите задумку текстом — текстовый путь работает всегда.',
        );
        return;
      }
