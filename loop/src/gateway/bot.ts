...
export function createTelegramGateway(deps: GatewayDeps): TelegramGateway {
  ...
  const onEvent = (event: LoopEvent): void => {
    ...
    if (event.type === 'feasibility') {
      /*
       * Суждение о выполнимости — ПЕРВЫМ сообщением, до сборки (А-42 п.2а; А-39).
       *
       * Кнопок нет намеренно, и это не упущение: вопрос владельцу здесь не задают. О материале и
       * разрешениях контур решает сам и объясняет (А-39), а «делай максимум» означает, что работа
       * пойдёт при любом вердикте. Человек получает объяснение ДО продукта — ровно то, чего не
       * хватало прошлым раундам, где расхождения всплывали после показа.
       */
      void send(
        alertText(
          event.verdict === 'полностью'
            ? '🔎 Задумка выполнима — берусь'
            : '🔎 Что выйдет и что не выйдет — до начала работы',
          projectLabel(event.projectId),
    ...
    if (event.type === 'quality') {
      /*
       * Суд качества — стадия, и его вердикт владелец читает ДОСЛОВНО (А-44 п.2). Зелёный приходит
