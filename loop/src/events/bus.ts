...
export type LoopEvent =
  ...
  /**
   * Вердикт суда качества (А-44 п.2): шлюз превращает его в алерт владельцу.
   *
   * Заведён вместе с самим судом и по прямой необходимости: пока «завершён» говорилось по счёту
   * задач, владелец узнавал итог из `project-status`. Теперь итог может быть «все задачи приняты,
   * а продукт не принят», и без своего события это была бы ТИШИНА — то есть ровно тот режим отказа,
   * который карта зовёт «проход стоит молча», только с другой стороны.
   */
  | {
      type: 'quality';
      projectId: string;
      green: boolean;
      /**
       * Какой из четырёх исходов случился (А-51 п.4).
       *
       * `green` — судили, зелено; `red` — судили, красная ось; `not-held` — суд не состоялся
       * (отказ контура); `unjudgeable` — продукт не судим этим судом (долг, вердикт §10.1).
       * Перечень повторён здесь строками намеренно: шина не зависит от модуля гейта, а разница
       * между «красный» и «не состоялся» обязана доезжать до владельца — прежде обе читались
       * одинаково, и алерт называл красным то, чего не смотрели.
       */
      kind: 'green' | 'red' | 'not-held' | 'unjudgeable';
      text: string;
    };
