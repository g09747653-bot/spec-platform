import {
  ...
  SELF_CHECK_FILE,
  SELF_CHECK_RECORD,
} from '../orchestrator/self-check.ts';
...
export function executorPrompt(taskFile: string, taskId: string): string {
  return [
    ...
    '',
    'ЗАПРЕТ, У КОТОРОГО НЕТ ИСКЛЮЧЕНИЙ: продукт не разговаривает с пользователем о своей',
    'незавершённости. Ни «демо-версия», ни «функция недоступна», ни «coming soon», ни «в разработке»,',
    'ни тостов и всплывающих сообщений об этом, ни отдельных файлов, которые такие сообщения',
    'показывают. Такой файл удаляется, а не переписывается. Честность живёт в отчёте, а не в',
    'интерфейсе: элемент, которому нечего делать, либо делает что-то настоящее, либо его нет.',
    '',
    `Расхождения с задумкой записывай ДВУМЯ файлами в корне рабочей директории — прозой для`,
    `человека и машинной записью для контура. Проза: ${SELF_CHECK_FILE}, ровно два раздела, оба`,
    `таблицами: «## I. ${REGISTRY_SECTIONS.material}» — что не воспроизводится, причина`,
    '(материал / лицензия / доступ / техника) и что поставлено взамен; сюда попадает только то, что',
    `названо условиями выполнимости ДО сборки. «## II. ${REGISTRY_SECTIONS.scope}» — что решено не`,
    'доводить до рабочего состояния при наличном материале: декоративные ссылки, незаполненные',
    'разделы, урезанные списки. Второй раздел законен и обязателен: сокращение объёма, не записанное',
    'туда, — это заглушка.',
    '',
    `Машинная запись: ${SELF_CHECK_RECORD} рядом с прозой, вида`,
    '{"entries":[{"kind":"материал","what":"…","why":"…","instead":"…"},',
    ' {"kind":"объём","what":"…","why":"…"}]}.',
    `Поле kind — РОВНО одно из двух: "${DEVIATION_KINDS[0]}" или "${DEVIATION_KINDS[1]}"; поле`,
    'instead — только у рода "материал" (замена сокращённому пункту и есть заглушка). Одна запись',
    'на каждую строку прозы, ни больше ни меньше. Контур считает роды ИМЕННО по этому полю: реестр,',
    'у которого рода нет полем, объявляется нераспознанной формой и числа по родам по нему не',
    'называются вовсе. Числа публикуются раздельно и воротами не становятся.',
    '',
