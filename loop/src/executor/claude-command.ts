import { REGISTRY_SECTIONS, SELF_CHECK_FILE } from '../orchestrator/self-check.ts';
...
export function executorPrompt(taskFile: string, taskId: string): string {
  return [
    ...
    '',
    `Расхождения с задумкой записывай в ${SELF_CHECK_FILE} в корне рабочей директории — ровно два`,
    `раздела, оба таблицами: «## I. ${REGISTRY_SECTIONS.material}» — что не воспроизводится, причина`,
    ...
    'туда, — это заглушка. Числа по разделам публикуются раздельно и воротами не становятся.',
    '',
