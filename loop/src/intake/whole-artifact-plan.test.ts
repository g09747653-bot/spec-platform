...
describe('промпт ветки', () => {
  ...
  /**
   * Условия выполнимости в промпте (А-42 п.2б).
   *
   * Без них полировка гонится за недостижимым: задача «сверь с эталоном и исправь расхождения»
   * будет вечно исправлять то, под что нет материала. Тот же урок, что D-323, с другой стороны.
   */
  it('условия выполнимости едут в промпт запретом и заменой', () => {
    const prompt = wholeArtifactPrompt(SEED, {
      ...context,
      conditions: [
        'фирменное начертание — не воспроизводится (лицензия); вместо этого: системный гротеск',
      ],
    });

    expect(prompt).toContain('УСЛОВИЯ ВЫПОЛНИМОСТИ');
    expect(prompt).toContain('1. фирменное начертание — не воспроизводится (лицензия)');
    expect(prompt, 'запрет без замены оставил бы исполнителя без задачи').toContain(
      'Взамен ставь то,',
    );
    expect(prompt).toContain('не должна этого требовать');
  });

  it('условий нет — блока в промпте нет: пустой раздел это шум', () => {
    expect(wholeArtifactPrompt(SEED, context)).not.toContain('УСЛОВИЯ ВЫПОЛНИМОСТИ');
    expect(wholeArtifactPrompt(SEED, { ...context, conditions: [] })).not.toContain(
