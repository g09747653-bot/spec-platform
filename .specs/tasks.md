...
- [x] 158\. M15а gate — live single-cycle walk (self-run, А-2.1)
...
### Milestone 16а — Параллельность и «Красный CI» (бандл A0: M3; детализировано Архитектором 2026-08-20 при закрытии M15а, амендмент А-25)

> Порядок сессии: долги 170 → 171 → 172 → 173 первыми, затем ядро 159 → 160 → 161 → 162 → 163. Параллельность строится и меряется штатно на подписке со всеми возможностями (А-24 п.2); троттлинг тарифа — штатный режим работы, не ошибка; каскадный отказ на троттлинге — дефект.

- [x] 159\. Parallel scheduler — до 10 исполнителей без коллизий
  - Вехи выполняются строго по `dependsOn`; ВНУТРИ вехи задачи идут параллельно, до `MAX_EXECUTORS` (конфиг, умолчание 10) контейнеров. Коллизии по `filesToEdit`: пересечение с уже запущенной задачей → задача ждёт очереди. Троттлинг тарифа: `rate_limit_event` со статусом, отличным от `allowed`, → конвейер не стартует НОВЫЕ контейнеры до момента сброса окна (запущенные доигрывают), лента объявляет паузу и время возобновления; фактическая параллель ≤ конфигу — законно.
  - Acceptance Criteria: юнит планировщика (конфликтующие ждут; неконфликтующие одной вехи параллельны; порядок вех строгий); стаб-тест троттлинга (не-allowed → пауза новых запусков → возобновление по сбросу); конкурентный SQLite-тест расширен на N параллельных циклов — ноль `database is locked`.
  - _Dependencies: —_ · _Requirements: бандл A0 (Task 3.1); А-23 п.4, А-24 п.2_ · _Touches: `loop/**`_ · _Complexity: Large_ · _Parallel-safe: no_

- [x] 160\. Красный CI — hard stop, заморозка, retry
  - Любой красный вердикт приёмки (тесты чистого контейнера, artifact walks, находки контролёра) → hard stop всей оркестрации: активные задачи → `PAUSED` на диске и в БД; `docker pause` контейнеров `delivery-executor-${taskId}` в течение 1 с; новые запуски не стартуют; дашборд показывает стоп-состояние с причиной (какая задача, какой тест упал). `POST /api/orchestrator/retry` (+ кнопка в дашборде): `docker unpause`, `PAUSED` → прежние статусы, красная задача перепроверяется заново.
  - Acceptance Criteria: e2e на стабах — инсценированный красный на одной из ≥3 параллельных → все контейнеры paused ≤1 с (docker inspect), retry возобновляет и доводит до конца; `PAUSED` переживает рестарт сервера и НЕ самовозобновляется — только retry.
  - _Dependencies: 159_ · _Requirements: бандл A0 (Task 3.2); Overview «Красный CI — полная остановка»_ · _Touches: `loop/**`_ · _Complexity: Large_ · _Parallel-safe: no_

- [x] 161\. Контроллёры и Исследователи — per-role провайдер (А-24)
  - Контролёр: после отчёта `SUCCESS`, ДО тестов приёмки — линтеры/форматтеры по стеку внутри контейнера (nodejs: eslint+prettier; python: flake8+black; go: fmt+vet; rust: fmt+clippy); красная проверка = возврат задачи исполнителю БЕЗ запуска тестов, находки в ленту. Исследователь: структурированный контекст-отчёт по workspace (дерево, манифесты, зависимости) — в контекст архитектора заданий при интейке и при возврате. Провайдер per-role: умолчание — та же учётка, что у исполнителя (подписка Claude); опции конфигурации — другой облачный или локальный (А-24 п.1); вызовы ролей точечные, из процесса оркестратора, без собственных контейнеров (экономия — бандл A0 Task 3.3).
  - Acceptance Criteria: цикл со стабами: ошибка форматирования → возврат до тестов; отчёт исследователя фикстурно присутствует в промпте заданий; переключение провайдера роли конфигом покрыто тестом (роль на google при исполнителе на подписке).
  - _Dependencies: 159_ · _Requirements: бандл A0 (Task 3.3); А-24 п.1_ · _Touches: `loop/**`_ · _Complexity: Medium_ · _Parallel-safe: no_

- [x] 162\. Полное восстановление с диска (Шаг 0) + gate-only VRAM-аудит
  - Пустая/удалённая SQLite при старте → полное восстановление: запись проекта (`INSERT OR REPLACE`, Шаг 0 против FK), вехи из `milestones.json`, задачи из `task_*.json`; автодетект утерянного `techStack` перед вставкой (алгоритм задачи 157); статусы дисковых файлов — истина (`PAUSED`/`BLOCKED` переживают рестарт и не самовозобновляются). VRAM-аудит: скрипт `vram_monitor` (PowerShell, `nvidia-smi`, шаг 500 мс, порог 12288 МБ) запускается гейтом 163 на машине заказчика параллельно прогону, пик — в артефакты; CI без GPU — именованное ограничение (gate-only).
  - Acceptance Criteria: тест «удалить `local.db` → старт → проект/вехи/задачи восстановлены без FK-ошибок, статусы файлов сохранены»; тест «`PAUSED` на диске → рестарт → стоит до retry»; скрипт аудита в репозитории, гейт его запускает.
  - _Dependencies: 160_ · _Requirements: бандл A0 (Task 3.4, 3.5); Success Criteria «Полное восстановление»_ · _Touches: `loop/**`, `loop/scripts/**`_ · _Complexity: Medium_ · _Parallel-safe: no_

- [x] 163\. M16а gate — живой параллельный прогон (self-run, А-2.1)
  - Настоящий небольшой продукт end-to-end при **≥3 параллельных живых исполнителях** на подписке (штатно, все возможности). Бандл — машинный экспорт платформы; если в текущем нет вехи с ≥3 непересекающимися задачами — произвести новый автономный прогон платформы с seed, спроектированным под параллель (несколько независимых модулей). Посреди прогона: (а) инсценированный красный CI (подложенный падающий тест одной задачи) → полная заморозка → retry → до конца; (б) SIGKILL оркестратора при работающей параллели → рестарт → возобновление. Замеры в артефакты: фактическая параллель во времени; все `rate_limit_event` (статусы, паузы); распределение длительностей итераций (данные для решения по потолку 5 мин — §7.4 рапорта M15а); пиковый VRAM. Красные условия: прежние + утечка учётки + `database is locked` + каскадный отказ на троттлинге.
  - Acceptance Criteria: прогулка GREEN, артефакты `artifacts/gate-M16a/` (RESULT, замеры, снимки, handoff-дерево, ленты, vram-пик); проверка Архитектора, затем приёмка заказчика глазами.
  - _Dependencies: 159–162, 170–173_ · _Requirements: А-2.1; А-24 п.2_ · _Touches: `loop/e2e/**`, `artifacts/gate-M16a/**`_ · _Complexity: Large_ · _Parallel-safe: no_

Платформенные и контурные долги из находок M15а — первыми в M16а-сессии, по образцу 168/169 (170/171 — А-22; 172/173 — А-25):

- [x] 170\. Driver survives a stack restart landing inside a model call (платформа; находка M15а §4.1)
  - Изолировать до конца и закрыть: рестарт стека на collect-позиции посреди вызова модели, которым драйвер запрашивает раунд, оставляет прогон в состоянии, где два тика подряд «ничего не изменили» → `stalled` (2 прогона из 2 на непустом профиле; без рестарта — зелёный). Ожидание: после подъёма политика обязана ПОВТОРИТЬ прерванный ход (в этом состоянии ask-round — законный ход, меняющий состояние), а не счесть себя петлёй; «нечего записать, потому что процесс умер посреди вызова» — не холостой шаг. Первый шаг — точная изоляция; улика — каталог `.local/db-m15a-stalled-040556` (не удалять до закрытия задачи).
  - Acceptance Criteria: регрессия, воспроизводящая обрыв (остановка стека посреди вызова модели или его стаба) — после рестарта прогон доходит до честной концовки; прогулка M14а с включённым рестартом зелёная три прогона подряд.
  - _Dependencies: —_ · _Requirements: задача 149 (claim «рестарт стоит ничего»); D-262_ · _Touches: `src/**` (autonomous policy/step)_ · _Complexity: Medium_ · _Parallel-safe: yes_

- [x] 171\. Walk bookkeeping scoped by project (находка M15а §4.1, побочная)
  - Бухгалтерия `e2e/gate-M14a.live.ts` (строка `autonomous_runs`, список ревизий, счёт проектов) читается по проекту прогулки, а не по владельцу: прогулка обязана быть перезапускаемой на обжитом локальном профиле без отвода каталога — «каталог отводить» как правило эксплуатации отвергнуто (ловушка для заказчика).
  - Acceptance Criteria: два прогона подряд на непустом локальном профиле дают независимые чистые отчёты; прогон на пустом профиле байт-в-байт прежний.
  - _Dependencies: —_ · _Requirements: D-262_ · _Touches: `e2e/gate-M14a.live.ts`_ · _Complexity: Small_ · _Parallel-safe: yes_

- [x] 172\. Intake idempotent over prose + живое умолчание модели заданий (вердикты §7.1/§7.2 рапорта M15а, А-25)
  - Существующий `task_*.json` сохраняет свои тексты (title, description, `filesToEdit`) при повторном интейке — то, по чему исполнитель мог начать работать, не переписывается (расширение D-261 с `status` на прозу); дополняются только отсутствующие поля (как `techStack`). Полная перегенерация — только явным действием оператора (отдельный параметр/эндпоинт с подтверждением), стирает handoff-дерево целиком и отказывает, если есть задача в работе. Заодно: умолчание модели роли «архитектор заданий» в коде сменить с мёртвого (`gemini-2.5-flash`, 404 на чистой машине) на живое; конфиг переопределяет.
  - Acceptance Criteria: повторный `start-loop` не меняет ни байта существующих заданий, кроме дополняемых полей; полная перегенерация работает с подтверждением и отказывает при задаче в работе; умолчание модели живое (тест на конфиг).
  - _Dependencies: —_ · _Requirements: рапорт M15а §7.1–7.2; D-261/D-229_ · _Touches: `loop/**` (intake)_ · _Complexity: Small_ · _Parallel-safe: yes_

- [x] 173\. Loop e2e в CI + общий словарь ожидаемого консольного шума (вердикты §7.5/§6.1 рапорта M15а, А-25)
  - Шаг `pnpm loop:e2e` добавляется в СУЩЕСТВУЮЩУЮ джобу «Lint, boundaries, types, unit» (имя обязательного чека не меняется — D-202; +установка chromium в шаг). Словарь ожидаемых событий обрыва запросов (формулировки Chromium/Firefox/WebKit из D-276) выносится в один модуль; пять live-скриптов (`gate-M12/M13/M14a/dogfood-A0/gate-M15a`) читают его оттуда — одна истина в одном месте.
  - Acceptance Criteria: CI гоняет 7 браузерных кейсов контура зелёными в той же джобе; live-скрипты импортируют общий модуль (копии удалены диффом); строгость набора не снижена (якоря формулировок сохранены).
  - _Dependencies: —_ · _Requirements: рапорт M15а §7.5, §6.1; D-276; D-202_ · _Touches: `.github/workflows/**`, `e2e/**`, `loop/e2e/**`_ · _Complexity: Small_ · _Parallel-safe: yes_

- [x] 174\. Пределы времени — по данным гейта 163 (вердикт §7.1/§7.2 рапорта M16а, А-26)
  - **Потолок итерации — задний упор, а не рабочий предел** (довод D-234): умолчание `ITERATION_TIMEOUT_MS` поднимается 300 с → **900 с** (3× p90 живого распределения: 26 итераций, медиана ~105 с, p90 ~300 с, три честные итерации убиты старым потолком); в схему задачи добавляется НЕОБЯЗАТЕЛЬНОЕ поле `iterationTimeoutSec` (аддитивно — компатибилити-контракт как у D-223: старые файлы валидны), архитектор заданий или оператор может пометить тяжёлую задачу. Вариант «нет вывода N секунд» вместо стенных часов ОТВЕРГНУТ сейчас (урок Р-4/А-9: молчание — не смерть; длинное рассуждение модели молчит законно) — вернуться только по данным реальных проектов.
  - **Свой таймаут прогона тестов приёмки**: команда тестов получает собственный предел (умолчание 300 с, конфиг `ACCEPTANCE_TEST_TIMEOUT_MS`), по превышении — красный вердикт с именованной причиной «тесты не завершились» (класс «тест, который не заканчивается», задача 17 гейта); пятнадцатиминутный потолок приёмки остаётся задним упором.
  - Acceptance Criteria: регрессия на новое умолчание и per-task поле; регрессия «незавершающиеся тесты краснеют за свой предел, приёмка не ждёт 15 минут»; распределение длительностей гейта 163 р.2 пишется в артефакты тем же скриптом.
  - _Dependencies: —_ · _Requirements: рапорт M16а §6.2, §7.1–7.2; measurements.json_ · _Touches: `loop/**` (executor wrapper, acceptance, task schema)_ · _Complexity: Small_ · _Parallel-safe: yes_

### Milestone 17а — Telegram и голос (бандл A0: M4; ТГ — последним по А-7 §3; детализировано Архитектором 2026-08-21 при закрытии M16а, амендмент А-27)

> Финальный milestone задумки. Порядок сессии: мелочь §7.5 рапорта M16а (`GATE_RESTART=0` не пишет PROBLEM — первой, чтобы не забылась) → 175 → 176 → 164 → 165 → 166 → 167. Гейт 167 закрывает текстовый сквозной путь сам; **голосовая задумка живьём — финальная приёмка заказчиком всей Программы А**.

- [x] 175\. Адаптер подписки штатно — `claude-cli` звено (вердикт §7.1 рапорта M16а)
  - Мост из scratchpad становится модулем репозитория: локальный OpenAI-совместимый сервер поверх Claude Code CLI (подписочный токен; три урока D-304 закреплены кодом: системная инструкция — только `--append-system-prompt`, промпт — потоком stdin, `--tools ''`), поднимается командой пакета. Платформа подключает его штатным конфигом локального звена (как мост и делал — ни одной новой строки в платформе); контур получает его же опцией per-role (шов задачи 161). Цепочка гейтов платформы отныне: адаптер спереди, google запасным, ollama локальным (вердикт §7.2). Р-1 остаётся стражем; грамматика А-10 к этому звену неприменима (облачный путь); значение токена не печатается нигде — красное условие.
  - Acceptance Criteria: юниты на три урока D-304; живая проба — платформенная генерация одного документа через адаптер; конфиг-тест цепочки; проверка утечки в артефакты.
  - _Dependencies: —_ · _Requirements: рапорт M16а §4/§7.1–7.2; D-304; А-23_ · _Touches: `loop/**` (bridge module), `.env.example`, docs_ · _Complexity: Medium_ · _Parallel-safe: no_

- [x] 176\. Причина красноты — исполнителю; POSIX-команды — в инструкцию (находки §6 рапорта M16а)
  - Возврат красной задачи несёт исполнителю причину: вердикт приёмки (упавший тест, вывод, находки контролёра) попадает в контекст повторной итерации (по образцу контролёрного возврата 161). «SUCCESS без единой правки файлов при красной причине» — именованный отказ повтора, а не третий одинаковый заход (mtime-класс прогона Б). Инструкция архитектора заданий получает строку «команды тестов — POSIX sh» (модельные PowerShell-команды прогона Б валились кодом 2 честно, но зря).
  - Acceptance Criteria: стаб-цикл — повтор видит причину (фикстурно в промпте); повтор без правок при красной причине → именованный отказ; строка инструкции закреплена тестом фикстуры.
  - _Dependencies: —_ · _Requirements: рапорт M16а §6; D-306_ · _Touches: `loop/**` (acceptance, executor prompt), prompt layer контура_ · _Complexity: Small_ · _Parallel-safe: yes_

- [x] 164\. Telegram-бот — закрытый, с полноценным интерфейсом (объём расширен А-28 п.4)
  - Клиент Bot API по long polling; обрабатываются только сообщения `TELEGRAM_OWNER_CHAT_ID` (из `chat_id.txt` заказчика — А-28 п.1), все прочие игнорируются молча, включая `/start`. **Полноценный интерфейс, не голый чат**: меню команд (`setMyCommands`), `/start` — русский онбординг (что умеет, как прислать задумку текстом или голосом), `/status` — текущий проект (вехи/задачи/состояние конвейера), `/stop` — остановка конвейера из чата (суверенитет), inline-кнопки подтверждений, структурированные сообщения постоянной формы. Алерты по событиям шины: запуск проекта, блокировка (текст из `BLOCKED_<taskId>.md`), красный CI, возобновление, успех. Бот — часть `loop/`; без токена контур работает как раньше (именованная опциональность).
  - Acceptance Criteria: интеграционный тест с фейковым Bot API (long-poll, фильтр владельца — чужой `/start` без ответа, команды, кнопки, алерты по каждому событию); значение токена не печатается (красное условие).
  - _Dependencies: 175_ · _Requirements: бандл A0 (Task 4.1, 4.4); А-28 п.4–5_ · _Touches: `loop/**` (gateway)_ · _Complexity: Large_ · _Parallel-safe: no_

- [x] 165\. Голос — зонд выполнен; ОТЛОЖЕН решением заказчика (пере-скоуплено А-29 по итогам зонда А-28 п.2)
  - Зонд выполнен раундом 1 (три именованных отказа подписочного тракта; google-замер положительный — `artifacts/probe-165/`); **вердикт заказчика: вариант (в) — голос отложить**. Остаток задачи — одно именованное поведение: голосовое сообщение от владельца получает ответ «голос отложен решением владельца — пришлите задумку текстом»; хук `transcribe` остаётся неподключённым; подключение звена — Backlog-строкой «по отдельному решению заказчика» (google — замер есть; Whisper — опция конфига). Текстовый путь — единственный вход задумки (равноправие уже построено в 164).
  - Acceptance Criteria: тест — голосовое от владельца → именованный ответ, никаких действий; Backlog-строка добавлена; фикстурная транскрибация СНЯТА вместе с выбором пути (А-29 п.3).
  - _Dependencies: 164_ · _Requirements: А-28 п.2; А-29 п.2–3; artifacts/probe-165/_ · _Touches: `loop/**` (gateway)_ · _Complexity: Small_ · _Parallel-safe: no_

- [x] 166\. Фасад: задумка → платформа → бандл → контур, поверх реальных поверхностей
  - Подтверждённый текст задумки запускает оркестрацию фасада в `loop/`: платформа локального профиля — создать проект+сессию из seed и включить автономный режим СУЩЕСТВУЮЩИМИ эндпоинтами (А-20 п.3в: не новый генератор); ожидание `complete`; машинный экспорт → `bundle/` в новый workspace-каталог; `start-loop`. Прогресс и отказы каждого звена — именованными алертами в TG (таймауты названы, молчания нет). Платформа не меняется ни строкой.
  - Acceptance Criteria: интеграционный тест цепочки вызовов фасада (стабы звеньев, фикстуры ответов); отказ любого звена даёт именованный алерт; живой путь целиком — гейт 167.
  - _Dependencies: 165_ · _Requirements: бандл A0 (Task 4.3); А-20 п.3в_ · _Touches: `loop/**` (gateway)_ · _Complexity: Medium_ · _Parallel-safe: no_

- [x] 167\. M17а gate — сквозной путь без рук (self-run, А-2.1; правлено А-29: приёмка текстом)
  - Живой прогон: ТЕКСТОВОЕ сообщение боту от владельца → платформа автономно (адаптер 175) → бандл → контур (живые исполнители на подписке) → готовый протестированный продукт → алерт успеха; ни одного действия руками после отправки сообщения (подтверждающая кнопка «Запустить» — единственное намеренное касание, и оно часть входа). Плюс проверка отложенного голоса: голосовое → именованный ответ 165. Красные условия все прежние: утечки учёток, `database is locked`, каскад на троттлинге, зависания, молчаливые отказы звеньев. Артефакты `artifacts/gate-M17a/` (RESULT, лента TG-алертов, замеры, handoff-дерево, скрины).
  - **Финальная приёмка milestone и всей Программы А — заказчик ТЕКСТОМ** (А-29 п.4): прислать боту собственную задумку и получить продукт; инструкция — три строки в README. (Голосовая версия финала вернётся, если заказчик вернёт голос.)
  - Acceptance Criteria: GREEN сквозного текстового прогона; именованный ответ на голосовое в том же прогоне; README-раздел для приёмки; проверка Архитектора, затем приёмка заказчика.
  - _Dependencies: 164–166, 175, 176_ · _Requirements: А-2.1; North Star этап 2; А-29_ · _Touches: `loop/e2e/**`, `artifacts/gate-M17a/**`_ · _Complexity: Large_ · _Parallel-safe: no_

### Финализация Программы А (Architect, 2026-08-22, А-31; сессия — после финальной приёмки заказчика)

- [ ] 177\. Течь слушателей на транспорте Docker-логов (вердикт §6.1 рапорта M17а р.3)
  - `MaxListenersExceededWarning` растёт с длиной конвейера (0 → 14 → 50 за девять попыток; корреляция с числом полных циклов, не с шириной): найти и закрыть накопление `drain`-слушателей потоков логов контейнеров (переиспользование/закрытие при завершении контейнера).
  - Acceptance Criteria: длинный стаб-прогон (≥23 цикла) — ноль предупреждений; живые наборы зелёные.
  - _Dependencies: —_ · _Requirements: рапорт M17а р.3 §6.1; Backlog-строка А-27_ · _Touches: `loop/**` (docker log streams)_ · _Complexity: Small_ · _Parallel-safe: yes_

- [ ] 178\. Последние хостовые глаза — под принцип А-30 (вердикт §6.2 рапорта M17а р.3)
  - Три оставшихся чтения контейнерных записей хостом (`readReport`, `existsSync` BLOCKED-файла в цикле, фильтр `judged` контролёра) переводятся на контейнерное наблюдение/снимки класса `observe.ts` — принцип «всё, что судит приёмка, наблюдается изнутри» не терпит формальных исключений, даже переживших девять попыток.
  - Acceptance Criteria: регрессии на шов наблюдателя для всех трёх мест; стаб- и контейнерные наборы зелёные; хостовый fs в цикле приёмки не читается нигде, кроме recovery при буте (законный потребитель по D-314).
  - _Dependencies: —_ · _Requirements: рапорт M17а р.3 §6.2; А-30_ · _Touches: `loop/**` (gate)_ · _Complexity: Small_ · _Parallel-safe: yes_

- [ ] 179\. Семантика перезахода завершённого проекта (находки fix-раунда §5.1 и до-захода §5.1; А-34)
  - Повторный `start-loop` тем же projectId получает осознанную семантику вместо унаследованных статусов: named-условие «дерево на диске всё PENDING, а индекс несёт терминальные статусы того же проекта → пересинхронизация с диска» (диск — истина; снос индекса руками перестаёт быть операторским обходом); плюс named-обновление статуса проекта в индексе при событии запуска (доска перестаёт врать COMPLETED во время живого прогона).
  - Acceptance Criteria: тест перезахода поверх терминального индекса (пересинхронизация, конвейер едет); тест статуса доски при событии запуска; `ON CONFLICT`-семантика D-261 для остальных полей не тронута.
  - _Dependencies: —_ · _Requirements: рапорт «Fix-раунд» §5.1; рапорт «До-заход» §5.1; D-261_ · _Touches: `loop/**` (intake, index)_ · _Complexity: Small_ · _Parallel-safe: yes_

- [x] 180\. ~~Команды приёмки — под slim-образы~~ — **ОТМЕНЕНА вердиктом А-44 п.1 и заменена обратным решением**
  - Прежняя формулировка («команды приёмки — без git и браузера; браузерные прогоны — работа исполнителя в итерации») лечила симптом и закрепляла причину: она делала штатным то, что приёмка не может произвести сама, — а значит принимает чужое. Живой прогон А-43 показал цену: `browserType.launch: Executable doesn't exist` трижды, и все три раза приёмка осталась зелёной.
  - Заменено принципом V1 конституции проверок (A-50): **приёмка не принимает артефакт, который она не могла произвести сама**; приёмочный образ обязан уметь то, что задача требует измерить, и рост образа есть названная цена независимости. Реализовано в D-330/D-331: `capability` на задании, образ с браузером собирается контуром, «не проверяемо приёмкой» — явная пометка.
  - _Dependencies: —_ · _Requirements: А-44 п.1; A-50_ · _Touches: `loop/**` (gate)_ · _Complexity: —_ · _Parallel-safe: —_
