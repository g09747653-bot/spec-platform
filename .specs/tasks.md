...
- [ ] 180\. Команды приёмки — под slim-образы (находка fix-раунда §5.2; А-34)
  - Инструкция архитектора заданий получает строку «команды приёмки — без git и браузера: file/grep/node --check/сборка; браузерные прогоны — работа исполнителя в итерации», закреплённую фикстурным тестом. Внутрифазовое правило нарезки (пороговые/сверочные задачи — в конец вехи) — Backlog-строкой, не этой задачей.
  - Acceptance Criteria: фикстурный тест инструкции; регрессия на слепке задания класса T022 (браузерная команда в приёмке → отсутствует у свежесгенерированного).
  - _Dependencies: —_ · _Requirements: рапорт «Fix-раунд» §5.2; А-30 §6 п.2_ · _Touches: prompt layer контура_ · _Complexity: Small_ · _Parallel-safe: yes_
