...
- [ ] 165\. Голос — сначала подписочным звеном, подтверждение кнопкой (правлено А-28 п.2)
  - **Первый шаг — зонд**: принимает ли подписочный путь (адаптер задачи 175 / CLI) аудиофайл и с каким качеством на русской речи (директива заказчика: «сначала на базе подписки, тем же образом»). Принимает → он умолчание транскрибации; `WHISPER_API_BASE` — запасная опция конфига. Не принимает → именованная остановка с вариантами, решение заказчика. Голосовое (.ogg/Opus) скачивается; конвертация `ffmpeg` — средство под формат звена (решает зонд). Распознанный текст бот показывает владельцу с кнопками «Запустить»/«Отмена» ДО каких-либо действий — распознанный текст есть решение владельца, а не догадка бота (дух P2). **Текстовый путь работает всегда, наравне с голосом** (А-28 п.3); неготовый голосовой конфиг отвечает именованно.
  - Acceptance Criteria: зонд записан в рапорт с образцами; фикстурный .ogg → текст тем путём, который выбрал зонд (интеграционно); подтверждение кнопкой запускает, отмена — нет; текстовый путь равноправен.
  - _Dependencies: 164, 175_ · _Requirements: бандл A0 (Task 4.2); А-28 п.2–3_ · _Touches: `loop/**` (gateway)_ · _Complexity: Medium_ · _Parallel-safe: no_

- [ ] 166\. Фасад: задумка → платформа → бандл → контур, поверх реальных поверхностей
...
- [ ] 167\. M17а gate — сквозной путь без рук (self-run, А-2.1)
  - Живой прогон: ТЕКСТОВОЕ сообщение боту от владельца → платформа автономно (адаптер 175) → бандл → контур (живые исполнители на подписке) → готовый протестированный продукт → алерт успеха; ни одного действия руками после отправки сообщения. Транскрибация проверяется фикстурным .ogg до текста тем же путём (живой голос — приёмка заказчика). Красные условия все прежние: утечки учёток, `database is locked`, каскад на троттлинге, зависания, молчаливые отказы звеньев. Артефакты `artifacts/gate-M17a/` (RESULT, лента TG-алертов, замеры, handoff-дерево, скрины).
  - Финальная приёмка milestone и всей Программы А — заказчик голосом: надиктовать задумку боту и получить продукт; инструкция для этого — три строки в README.
  - Acceptance Criteria: GREEN сквозного текстового прогона; фикстурная транскрибация в тот же путь; README-раздел для голосовой приёмки; проверка Архитектора, затем приёмка заказчика голосом.
  - _Dependencies: 164–166, 175, 176_ · _Requirements: А-2.1; North Star этап 2_ · _Touches: `loop/e2e/**`, `artifacts/gate-M17a/**`_ · _Complexity: Large_ · _Parallel-safe: no_
