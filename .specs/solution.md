...
**Chat decision routing.** `POST /api/sessions/:id/messages` carries `{ text }`. When a decision is pending, the handler calls `resolveDecisionIntent`; a resolved intent is **dispatched internally to the same decision endpoint the card would have called**, and the response reports which decision was applied. An unresolved intent returns the assistant's textual answer with `pendingAction` unchanged, so the client re-renders the same card (FR-009 AC-6/AC-7).
