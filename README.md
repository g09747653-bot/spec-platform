...
1. Поднимите всё: `pnpm local:up` (платформа), `pnpm --filter @spec-platform/loop bridge` (мост подписки), `pnpm loop:dev` (контур с заполненными `TELEGRAM_BOT_TOKEN`, `TELEGRAM_OWNER_CHAT_ID`, `SPEC_PLATFORM_API_BASE=http://127.0.0.1:3000`) — и Docker Desktop.
